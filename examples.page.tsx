...
export default function* (_data: Lume.Data, helpers: Lume.Helpers) {
  ...
  for (const example of examples) {
    ...
    const url =
      `https://github.com/denoland/deno-docs/blob/main/by-example/${example.name}${example.parsed.files.length > 1 ? "/main" : ""
      }`;
    const rawUrl =
      `https://docs.deno.com/by-example/${example.name}${example.parsed.files.length > 1 ? "/main" : ""
      }`;
