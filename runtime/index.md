...
[Additional installation options can be found here](./manual/getting_started/installation.md).
...
You can
[learn more about using TypeScript in Deno here](./manual/advanced/typescript/index.md).

## Built-in web APIs and the Deno namespace

Deno aims to provide a browser-like programming environment,
[implementing web standard APIs](./manual/runtime/web_platform_apis.md) that
exist in front-end JavaScript. For example, the
...
the [`Deno` namespace](./manual/runtime/builtin_apis.md). Replace the contents
of `hello.ts` with the following code, which will start
...
Learn more about the [web-standard APIs](./manual/runtime/web_platform_apis.md)
built in to Deno and the
[`Deno` namespace APIs](./manual/runtime/builtin_apis.md).

## Runtime security

A major feature of Deno is
[runtime security by default](./manual/basics/permissions.md), meaning that you
as the developer must explicitly allow your code to access potentially sensitive
APIs like file system access, network connectivity, and access to environment
variables.
...
[configurable runtime security options](./manual/basics/permissions.md).
...
[You can learn more about the module system in Deno here](./manual/basics/modules/index.md).
...
above. Deno provides a [built-in test runner](./manual/basics/testing/index.md),
which uses an assertion module distributed via HTTPS URL.
...
[`deno.json` or `deno.jsonc`](./manual/getting_started/configuration_file.md).
This file acts a bit like a `package.json` file in Node.js.

One of the things you can use `deno.json` for is configuring an
[import map](./manual/basics/import_maps.md), which will let you set up aliases
for frequently used modules. To demonstrate, let's pin the version of the
standard library we want to use in our project to version `0.200.0`.
...
Learn more about
[configuring your project here](./manual/getting_started/configuration_file.md).

## Node.js APIs and npm packages

Deno provides a compatibility layer that enables your code to use
[Node.js built-in modules and third-party modules from npm](./manual/node/index.md).
...
[Learn more about Node and npm support](./manual/node/index.md).
...
[Deno Deploy](/deploy). Deno Deploy makes it easy to create fast, globally
distributed serverless applications with Deno.

You can also host Deno
[in almost any cloud environment](./manual/advanced/deploying_deno/index.md).
...
- [Deno Manual](./manual/index.md) - a comprehensive overview of the Deno
  runtime
- [Tutorials and Examples](./tutorials/index.md) - Sample code and use cases for
  Deno
