...
description: "How to move a Yarn project to Deno: install dependencies from your existing package.json, map Yarn CLI commands to Deno, run workspaces unchanged, and what to do about Plug'n'Play."
...
project with a standard `package.json`, and Deno reads that directly: it installs
and resolves the same npm dependencies and runs your scripts. A typical Yarn
project runs under Deno with no changes.
...
does not read `yarn.lock`; it resolves your `package.json` dependencies fresh on
the first install.
...
In practice this means the PnP-specific files and settings stop applying once you
move to Deno:
...
- `yarn patch` has no built-in Deno equivalent. Vendor the dependency or keep the
  patch in a fork.
