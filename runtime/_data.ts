...
export async function generateNodeCompatability() {
  ...
  const sorted = Object.entries(descriptions).toSorted(([keyA], [keyB]) => keyA.localeCompare(keyB));
  const grouped: Record<string, { title: string; icon: string; items: Array<[string, DescriptionItem]> }> = {
    good: {
  ...
  return Object.entries(grouped).map(([_status, entries]) => {
    ...
    content += entries.items.map(([key, content]) => {
      let out =
        `\n\n### <a href="/api/node/${key}">node:${key.replaceAll("--", "/")}</a>\n\n<div class="item-content">\n\n`;
