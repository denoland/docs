...
export default function examplesPage({ examples }: Props) {
  examples = examples || [];
...
function generateExampleGroups(examples: ExampleFromFileSystem[]) {
  ...
  const groups: Record<string, ExampleFromFileSystem[]> = examples.reduce(
    (acc: Record<string, ExampleFromFileSystem[]>, example) => {
      ...
      if (
        !groupMap.find(([category]) => category.toLowerCase() === lowerGroup)
      ) {
