...
- App Router pages are still prerendered at build time when Next.js decides they
  ...
  code until after the component mounts, such as in `useEffect`, or load the
  component with `next/dynamic` and `{ ssr: false }`. Marking the route dynamic
  is not enough: dynamic routes still render on the server, where browser
