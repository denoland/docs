...
const SUBSEQUENT_ENCAPSULATED_ANSI_RE = new RegExp(`${ENCAPSULATED_ANSI_RE.source}( ?)${ENCAPSULATED_ANSI_RE.source}`, "g");
...
function renderCommand(commandName: string, helpers: Lume.Helpers): { rendered: any; toc: TableOfContentsItem_[]; } {
  const command = CLI_REFERENCE.subcommands.find((command) => command.name === commandName)!;

  const toc: TableOfContentsItem_[] = [];

  let about = command.about!.replaceAll(SUBSEQUENT_ENCAPSULATED_ANSI_RE,function (_, _opening1, text1, _closing1, space, opening2, text2, closing2) {
  ...
  about = aboutLines.join("\n").replaceAll(ENCAPSULATED_ANSI_RE, (_, opening, text, _closing, offset, string) => {
    if (opening === "\u001b[32m") { // green, used as heading
      return `### ${text}`;
    } else if (opening === "\u001b[38;5;245m" || opening === "\u001b[36m" || opening === "\u001b[1m" || opening === "\u001b[22m") { // gray and cyan used for code and snippets, and we treat yellow and bold as well as such
  ...
  const rendered = (
    <div>
      <div class="p-4 bg-stone-100 rounded border border-gray-300 mt-6 mb-6 relative">
        <h3 class="!text-xs !m-0 -top-2.5 bg-white border border-gray-600/25 px-2 py-0.5 rounded absolute !font-normal">Command-line Usage</h3>
        <div>
      ...
      {Object.entries(options).map(([heading, flags]) => {
        ...
        const renderedFlags = flags.map((flag) => renderOption(id, flag, helpers));
...
function renderOption(group: string, arg, helpers: Lume.Helpers) {
  ...
  const helpLinesDocsIndex = helpLines.findLastIndex((line) => line.toLowerCase()
    .trim()
    .startsWith("docs:"));
  ...
  return <>
    ...
    {arg.help && <p class="block !whitespace-pre-line" dangerouslySetInnerHTML={{
      __html: helpers.md(flagsToInlineCode(help) + ((help.endsWith(".") || help.endsWith("]"))
        ? ""
        : "."))
    }} />}
  </>
