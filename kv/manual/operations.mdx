...
The `watch` operation accepts a key or array of keys, and returns 
a `ReadableStream`, which emits a new value  whenever any of the 
watched keys change their `versionstamp`. The emitted value is an 
array of [Deno.KvEntryMaybe](https://deno.land/api?s=Deno.KvEntryMaybe&unstable=) objects.
