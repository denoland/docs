...
export default async function feedbackRequestHandler(req: Request): Promise<Response> {
    ...
    const enableFeedbackMiddleware = privateKey && serviceAccountEmail && spreadsheetId;

    if (!enableFeedbackMiddleware) {
        return new Response("Feedback API is not enabled - have you set the correct environment variables?", { status: 500 });
    }
